...
class _ChargePointHandler(CP16):
    ...
    async def _simulate_diagnostics(self):
        try:
            ...
            await self.charger._call(call.DiagnosticsStatusNotificationPayload(status=DiagnosticsStatus.uploading))
            await asyncio.sleep(random.uniform(3, 8))
            self.charger._diagnostics_status = DiagnosticsStatus.uploaded
            await self.charger._call(call.DiagnosticsStatusNotificationPayload(status=DiagnosticsStatus.uploaded))
